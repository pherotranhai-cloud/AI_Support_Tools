...
async function addAutoplay(buf, durations) {
  ...
  for (let i = 0; i < durations.length; i++) {
    ...
    const m = xml.match(/<p:cNvPr id="(\d+)" name="Narration"/);
    if (!m) continue;
    ...
    const t = timingXml(m[1], durations[i]);
    if (xml.includes('</p:clrMapOvr>')) xml = xml.replace('</p:clrMapOvr>', '</p:clrMapOvr>' + t);
    else xml = xml.replace('</p:cSld>', '</p:cSld>' + t);
    zip.file(path, xml);
  }
  const ct = zip.file('[Content_Types].xml');
  if (ct) zip.file('[Content_Types].xml', (await ct.async('string')).replace('ContentType="audio/mp3"', 'ContentType="audio/mpeg"'));
