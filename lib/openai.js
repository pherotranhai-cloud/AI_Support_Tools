...
async function withModels(kind, candidates, fn) {
  ...
  for (const model of order) {
    try {
      ...
      lastErr = e;
      // Model mới (vd gpt-6-luna) có thể từ chối tham số/kiểu input (400) hoặc cần quyền (403) → thử model kế tiếp
      if (e instanceof ApiError && (e.isModelProblem || e.status === 400 || e.status === 403)) continue;
      throw e;
    }
  }
  throw lastErr;
...
const TEXT_MODELS = () => list('TEXT_MODEL', ['gpt-6-luna', 'gpt-4o', 'gpt-4.1', 'gpt-4o-mini']);
const IMAGE_MODELS = () => list('IMAGE_MODEL', ['gpt-6-luna', 'gpt-image-1', 'gpt-image-1-mini', 'dall-e-3']);
const TTS_MODELS = () => list('TTS_MODEL', ['gpt-4o-mini-tts-2025-12-15', 'gpt-4o-mini-tts', 'tts-1-hd', 'tts-1']);
...
async function image(prompt, { landscape = true } = {}) {
  return withModels('image', IMAGE_MODELS(), async (model) => {
    ...
    if (isDalle) {
      ...
      payload.quality = process.env.IMAGE_QUALITY || 'low';
    }
