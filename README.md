...
| `TEXT_MODEL` | `gpt-4o` → `gpt-4.1` → `gpt-4o-mini` | model đọc đề/viết nội dung (cần hỗ trợ ảnh + PDF + JSON schema). Có thể ghi nhiều model, cách nhau dấu phẩy, web tự thử lần lượt |
| `IMAGE_MODEL` | `gpt-image-1` → `gpt-image-1-mini` → `dall-e-3` | gpt-image cần **Organization verification** trên platform.openai.com. Lỗi hết → slide dùng nền màu |
| `IMAGE_QUALITY` | `medium` | `low` nhanh/rẻ hơn |
| `TTS_MODEL` | `gpt-4o-mini-tts` → `tts-1-hd` → `tts-1` | giọng đọc |
